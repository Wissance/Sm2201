...
Данный проект имеет двойное лицензирование:

* Опэнсорс: GPL v3
* Коммерческое использование: Есди вы планируете использовать код данного репозитория без обязательств GPL свяжитесь с нами по эл. почте `info@wissance.com` для приобретения коммерческой лицензии.

### 1.2 EN

This project is dual-licensed:
* Open Source: GPL v3
* Commercial: If you need to use this code in a proprietary application without GPL obligations, contact us at `info@wissance.com` for a commercial license.

## 2. Состав решения

В состав данного решения входят:

* `CAMAC-контроллер` крейта (подходит для управления любым `CAMAC-оборудованием`)
* блок накопления мессбауэровского спектрометра
...
Для правильной работы требуется установить настройки COM-устройства следующим образом:

![Настройки COM-порта](./docs/sm2201/Контроллер_крейта/port_settings.png)

Для тестирования взаимодействия использовалась наша утилита [`Zerial`](https://github.com/Wissance/Zerial), но позднее будет:
* отдельная специализированная `GUI-утилита` для `CAMAC`
* SDK для использования (точно `Python`, возможно еще `.Net` и `LabVIEW`)
